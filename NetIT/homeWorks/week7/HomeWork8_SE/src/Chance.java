	...
	public static void chance() {
		...
		if (Board.boardPosition[SetUpPlayers.playerIndex.getPosition()].getFieldsName().
			...
			SetUpPlayers.myFrame.textInfoPlayerPosition.setText(" " + SetUpPlayers.playerIndex.getName() 
			                                                    + " попаднахте на поле *** ШАНС ***");
			
			
			if (!Steal.isStealFree && SetUpPlayers.playerIndex.isStealProtected() == true && Steal.choosePlan == 0) {
			
			JOptionPane.showInternalMessageDialog(null, "Получавате награда от злия си план");	
		...
		if (chanceIndex % 2 == 0) {
			...
			SetUpPlayers.myFrame.textInfoFields.setText(" " + SetUpPlayers.playerIndex.getName()
					+ " днес е радостен ден за вас \n"
					+ " получавате бонус " + goodChance() + " лв.");

		} else {
			
			SetUpPlayers.myFrame.textInfoFields.setText(" " + SetUpPlayers.playerIndex.getName()
					+ " изтеглихте късата клечка \n"
					+ " Губите " + badChance()+ " лв.");
